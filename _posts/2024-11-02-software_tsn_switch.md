...
Here, the network device enp2s0f1 supports 8 TX queues. For virtual network devices, as often used on hosts to connect VMs to the software bridge, you can define the number of TX queues as follows:
...
We set up a software bridge called br0 and bring it up:

```
todo
```

We assume that each namespace is attached to the virtual bridge with a virtual Ethernet (veth) interface. veth interfaces actually come as pairs of devices, like a virtual cable with two ends. One end will be attached to the container (or VM), the other end to the virtual bridge (like you would connect a physical host to a physical bridge). Since TSN requires VLAN tags to carry PCP values, we also create VLAN interfaces (VLAN id 100) for one end of the virtual cable (the end attached to the container). All packets coming out of this VLAN device (from the container to the bridge) will carry a VLAN tag; for all incoming packets (from the bridge to the container), the VLAN tag is removed. The sending application (called talker in the following) defines SKB priorities as described above using the SO_PRIORITY socket option. This SKB priority is mapped to the PCP value of the VLAN header using the `todo` option. This ensures that all packets arriving at the bridge have a VLAN tag with defined PCP value. This also applies to packets coming from the physical network, i.e., the bridge receives over all attached interfaces VLAN-tagges packets with PCP field.      
...
To perform traffic shaping on egress traffic, we need to assign TAPRIO QDiscs to all (egress) network interfaces. In our little example, we only define a TAPRIO QDisc for the virtual bridge interface towards namespace ns-2 to demonstrate the idea: 
...
Finally, we need to remember that the virtual bridge receives VLAN-tagges packets with PCP values, but TAPRIO needs SKB priorities to classify packets, i.e., we need to perform a mapping from PCP values to SKB priorities. A very elegant method working without removing the VLAN tag is to use a so-called ***ingress*** QDisc on all interfaces of the virtual bridge together with the action `todo`. This action does exactyl what its name suggests: modify the SKB. In our case, we specify a mapping from PCP value to SKB priority (for the sake of a short description, we only do it for the interface from namespace `ns-1`, but you typically would do this on all interfaces of the bridge):
...
Finally, we can test our software TSN bridge as we have set up above. We assume that the listener is located in namespace `ns-2` (since this is also the only namespace in our example for which we have set up TAPRIO). We start a talker in namespace `ns-1` sending UDP messages as fast as possible to the listener with priority todo. The gate for priority todo is open 50 ms and closed 100 ms. Therefore, we would expect to see this pattern in the traffic arriving at the talker.

The following figure shows the arrival times of packets at the listener. We draw a vertical line whenever a packet of stream is received by the listener (the individual lines blend together at higher data rates). As we can see, the packets arriving with the anticipated pattern in bursts of 50 ms length. 
